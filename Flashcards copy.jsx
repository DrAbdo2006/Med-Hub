...
// Quiz questions = imported MCQs (fixed options) + auto-generated from flashcards.
function buildQuiz(deck) {
  const fromMcqs = (deck.mcqs || []).map((m) => ({ id: m.id, q: m.q, answer: m.answer, options: shuffle([...m.options]), image: null }));
  const fromCards = deck.cards.length >= 3
    ? deck.cards.map((card) => {
        const others = deck.cards.filter((c) => c.id !== card.id).map((c) => c.a);
        return { id: card.id, q: card.q, answer: card.a, options: shuffle([card.a, ...shuffle(others).slice(0, 3)]), image: card.image };
      })
    : [];
  return [...fromMcqs, ...fromCards];
}
const quizCount = (deck) => (deck.mcqs?.length || 0) + (deck.cards.length >= 3 ? deck.cards.length : 0);
const canQuiz = (deck) => (deck.mcqs?.length || 0) > 0 || deck.cards.length >= 3;
...
export default function App() {
  ...
  // Deleting a file (folder) keeps its projects but moves them to "Uncategorized".
  function deleteFolder(folderId) {
    setFolders((p) => p.filter((f) => f.id !== folderId));
    setDecks((p) => p.map((d) => (d.folderId === folderId ? { ...d, folderId: null } : d)));
  }
  ...
  // Bulk-imported MCQs (from CSV)
  const importMcqs = (deckId, mcqs) => setDecks((prev) => prev.map((d) => (d.id === deckId ? { ...d, mcqs: [...(d.mcqs || []), ...mcqs] } : d)));
    ...
    <Shell>
      ...
      {openProject ? (
        ...
        <LibraryView
          ...
          onCreateFolder={createFolder} onRenameFolder={renameFolder} onSetFolderDesc={setFolderDesc} onDeleteFolder={deleteFolder} onPinFolder={togglePinFolder}
...
function Header({ inStudy, onBack, onSettings, minimal, backLabel = "Exit" }) {
  ...
  const backBtn = (
    <button onClick={onBack} title={backLabel} className={`flex items-center gap-1.5 rounded-lg border border-med-lines px-3 py-2 text-sm font-light shadow-sm transition active:scale-95 ${dark ? "bg-slate-800 text-slate-100 hover:bg-slate-700" : "bg-white text-med-text hover:bg-slate-50"}`}><ArrowLeft className="h-4 w-4" /> {backLabel}</button>
  );
  if (inStudy) {
    return (
...
function FolderTitle({ folder, onRename, editing, onEditingChange }) {
  if (!folder) return <span className="font-semibold text-slate-500">Uncategorized</span>;
  return (
...
function CardImage({ src, className = "" }) { return src ? <img src={src} alt="" className={`mx-auto max-h-44 rounded-xl object-contain ${className}`} /> : null; }
...
function CardForm({ initial, accent, onCancel, onSave, onBulk }) {
  ...
  function save() { if (!valid) return; onSave({ id: initial.id, q: q.trim(), a: a.trim(), image }); }
  ...
  return (
    <div className="rounded-2xl border-2 border-indigo-200 bg-indigo-50/40 p-5 shadow-sm">
      ...
      <Field label="Image (optional)">
        {image ? (
          <div className="flex items-center gap-3"><img src={image} alt="" className="h-16 w-16 rounded-lg border border-slate-200 object-cover" /><button onClick={() => setImage(null)} className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50"><X className="h-3.5 w-3.5" /> Remove</button></div>
        ) : (
          <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-500 hover:border-indigo-300 hover:text-indigo-600"><Upload className="h-4 w-4" /> Upload image<input type="file" accept="image/*" className="hidden" onChange={(e) => readImage(e.target.files?.[0], setImage)} /></label>
        )}
      </Field>
...
function StudyView({ deck, session, setSession, srs, settings, onReview, onRecord, onFinish }) {
  ...
  const grades = [
    { key: "hard", label: "Hard", btn: "bg-rose-100 hover:bg-rose-200 text-rose-700 border-rose-200" },
    { key: "good", label: "Good", btn: "bg-sky-100 hover:bg-sky-200 text-sky-700 border-sky-200" },
    { key: "easy", label: "Easy", btn: "bg-emerald-100 hover:bg-emerald-200 text-emerald-700 border-emerald-200" },
  ];
  function grade(g) {
      ...
      <div className="mx-auto w-full max-w-2xl cursor-pointer select-none" style={{ perspective: "1600px" }} onClick={() => setSession({ ...session, flipped: !session.flipped })}>
        <div className="grid w-full transition-transform duration-500" style={{ transformStyle: "preserve-3d", transform: session.flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
          ...
          <div className={`flex h-auto min-h-[14rem] flex-col items-center justify-center gap-4 rounded-3xl border border-slate-200 bg-gradient-to-br ${deck.accent} p-8 text-center shadow-xl`} style={{ gridArea: "1 / 1", backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
            ...
            {card.image && <img src={card.image} alt="" className="mx-auto max-h-32 rounded-xl object-contain ring-2 ring-white/40" />}
    ...
    <div>
      ...
      <div className="mx-auto w-full max-w-2xl">
        <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-xl">
          ...
          {item.image && <div className="mb-4"><CardImage src={item.image} /></div>}
  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-[11px] text-slate-500">
        ...
        <span className="flex items-center gap-1.5">{tot ? <><span className="text-emerald-600">{easy}E</span><span className="text-sky-600">{good}G</span><span className="text-rose-600">{hard}H</span></> : <span className="text-slate-400">{meta}</span>}</span>
      </div>
      <div className="flex h-2 w-full overflow-hidden rounded-full bg-slate-200">
        <div className="bg-emerald-400" style={{ width: `${(easy / (tot || 1)) * 100}%` }} />
        <div className="bg-sky-400" style={{ width: `${(good / (tot || 1)) * 100}%` }} />
        <div className="bg-rose-400" style={{ width: `${(hard / (tot || 1)) * 100}%` }} />
      </div>
    </div>
  );
}
...
function ProjectCard({ deck, imageCount, srs, onOpen, onRename, onDelete, onPin, onExport, onMove }) {
  ...
  return (
    <div className={`flex flex-col rounded-2xl border bg-white p-5 shadow-sm transition-all duration-300 ease-in-out hover:scale-[1.02] hover:shadow-md hover:border-med-primary ${deck.pinned ? "border-indigo-200 ring-1 ring-indigo-100" : "border-slate-200"}`}>
      <div className="mb-1 flex items-start justify-between gap-2">
        <EditableTitle value={deck.title} onChange={(t) => onRename(deck.id, t)} editing={renaming} onEditingChange={setRenaming} clickToEdit={false} className="font-semibold text-slate-900" />
        <div className="flex shrink-0 items-center gap-1.5">
          {deck.pinned && <Pin className="h-4 w-4 fill-indigo-500 text-indigo-500" />}
          <Menu items={[
      ...
      <p className="flex-1 text-xs text-slate-500">{deck.description}</p>
      {confirmDel && (
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-400">
        <span className="flex items-center gap-1"><Layers className="h-3.5 w-3.5" />{deck.cards.length}</span>
        <span className="flex items-center gap-1"><AlignLeft className="h-3.5 w-3.5" />{deck.gaps.length}</span>
        <span className="flex items-center gap-1"><ImageIcon className="h-3.5 w-3.5" />{imageCount}</span>
        <span className="flex items-center gap-1 text-indigo-500"><Clock className="h-3.5 w-3.5" />{due} due</span>
      </div>
      <button onClick={() => onOpen(deck.id)} className={`mt-4 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r ${deck.accent} px-4 py-2.5 text-sm font-semibold text-white shadow-md transition hover:opacity-90 active:scale-95`}><BookOpen className="h-4 w-4" /> Open project</button>
    </div>
  );
}
function LibraryView({ folders, decks, occlusions, srs, onOpen, onCreateProject, onRenameProject, onDeleteProject, onPinProject, onSetFolder, onCreateFolder, onRenameFolder, onSetFolderDesc, onDeleteFolder, onPinFolder, onImportProject }) {
  const dark = useContext(ThemeCtx);
  ...
  const groups = groupByFolder(folders, decks, true).sort((a, b) => {
    if (!a.folder) return 1; if (!b.folder) return -1; // Uncategorized stays last
    return ((b.folder.pinned ? 1 : 0) - (a.folder.pinned ? 1 : 0)) || (recency(b.folder.id) - recency(a.folder.id));
  });
  ...
  function submitProject() { if (!pname.trim()) return; onCreateProject(pname.trim(), pdesc.trim() || "Custom project.", pfolder || null); setPname(""); setPdesc(""); setPfolder(""); setCreatingProject(false); }
  ...
  return (
    <div>
      ...
      {visibleGroups.length === 0 && <p className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-400">No projects match “{query}”.</p>}
            ...
            <>
              ...
              {confirmDeleteId === g.folder.id ? (
                <span className="flex items-center gap-1.5">
                  <button onClick={() => setConfirmDeleteId(null)} className={`rounded-lg border px-3 py-2 text-sm font-medium transition ${dark ? "border-slate-600 bg-slate-800 text-slate-200 hover:bg-slate-700" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}>Cancel</button>
                  <button onClick={() => { onDeleteFolder(g.folder.id); setConfirmDeleteId(null); }} title="Projects move to Uncategorized" className="flex items-center gap-1.5 rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-700"><Trash2 className="h-3.5 w-3.5" /> Confirm</button>
                </span>
              ) : (
...
function ProjectView({ deck, occlusions, progress, lastProg, onBack, onRename, onSetDesc, onStudy, onAddCard, onEditCard, onDeleteCard, onAddGap, onEditGap, onDeleteGap, onImportMcqs, onDeleteMcq, onNewImage, onEditImage, onDeleteImage, onStudyImages }) {
  ...
  return (
    <div>
      <button onClick={onBack} className="mb-4 flex items-center gap-1.5 text-sm font-medium text-slate-500 transition hover:text-slate-900"><ArrowLeft className="h-4 w-4" /> Back to files</button>
            ...
            {deck.cards.map((card, i) => (
              <div key={card.id} className="flex items-start justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2.5">
                <div className="min-w-0">
                  <div className="flex items-center gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-slate-200 text-[11px] font-semibold text-slate-500">{i + 1}</span><p className="truncate text-sm font-medium text-slate-800">{card.q}</p>{card.image && <ImageIcon className="h-3 w-3 shrink-0 text-sky-500" />}</div>
      ...
      {/* Quiz — bulk-import MCQs from a CSV, plus auto-generated from flashcards */}
...
function CompleteView({ session, deck, total, onRestart, onHome }) {
  ...
  const stats = mode === "flip"
    ? [{ label: "Easy", value: session.results.easy, color: "text-emerald-600", bg: "bg-emerald-50" }, { label: "Good", value: session.results.good, color: "text-sky-600", bg: "bg-sky-50" }, { label: "Hard", value: session.results.hard, color: "text-rose-600", bg: "bg-rose-50" }]
